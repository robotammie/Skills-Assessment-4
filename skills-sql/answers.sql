...
SELECT models.name, models.brand_name, brands.founded FROM models LEFT OUTER JOIN brands ON models.brand_name=brands.name WHERE models.year=1960;
...
SELECT b.name, founded FROM brands AS b WHERE NOT EXISTS (SELECT * FROM models AS m WHERE b.name = m.brand_name);